...
def costFunctionReg(theta, X, y, labda):
    ...
    grad=np.zeros((np.size(theta),1),dtype=float)

    #setting predictions
    m=len(y)
    #calculating predictions using the sigmoid function
    predictions=sg.sigmoid(np.dot(X,theta))
    predictions=predictions.reshape(np.size(predictions),1,order='F')
    y=y.reshape(np.size(y),1,order='F')
    ...
    grad[1:endg,:]=((((1./m )* (np.dot(((predictionsminusy.conj().T)), (X[:,1:end]))))).conj().T) +((labda/m)*theta[1:endr]).reshape((endr-1),1,order='F');
